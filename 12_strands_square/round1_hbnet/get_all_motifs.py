...
for pdb_file in glob.glob("*.pdb"):
		...
		with open(net+".cst", 'r') as cst_file:
			...
			for line in cst_file:
				if "#network_" in line:
					...
					net_name = vals[2]
					if "Y_26" in net_name:
						Y26.append((net_name, i))
					elif "Y_114" in net_name:
						Y114.append((net_name, i))
				elif "#AtomPair" in line:
					cst = line[1:]
			nets[i] = (net_name, cst)
	prod = list(itertools.product(Y26, Y114))
	print(pdb_file, prod)
	for p in prod:
	...
	for key in nets:
		if len(nets[key][0].split(",")) == 4: # Add this line if only combinations of motifs are to be considered (exclude single motifs)
			...
			new_path = "../round2_fasol085/"+pdb_name+"_"+str(key)+"/"
			try:
				os.mkdir(new_path)
			except OSError:
			...
			with open(pdb_file, 'r') as template:
				with open(new_path+pdb_name+"_000"+str(key) + ".pdb", 'w') as out_f:
					for l in template:
